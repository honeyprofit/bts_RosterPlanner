...
function generateOptimizedSchedule_(employees, dateInfo, year, month, prevMonthData, workdayRequirements, dailyStaffRequirements) {
  ...
  // 규칙 12: SP, SSV는 is_rq 제외하고 모두 ON
  for (let name in employees) {
  ...
  // 규칙 10: is_rq는 반드시 OFF (정직원만)
  for (let name in employees) {
    const empType = employees[name].employment_type;
    if (empType !== "PARTNER" && empType !== "SP" && empType !== "SSV") {
      for (let d = 1; d <= daysInMonth; d++) {
        if (employees[name].days[d] && employees[name].days[d].is_rq) {
          schedule[name][d] = "OFF";
        }
  ...
  // is_available_day가 FALSE인 경우 OFF
  for (let name in employees) {
    const empType = employees[name].employment_type;
    if (empType !== "SP" && empType !== "SSV") {
      for (let d = 1; d <= daysInMonth; d++) {
  ...
  for (let name of regularEmployees) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      if (schedule[name][d] === "OFF" && employees[name].days[d]) {
        const dayData = employees[name].days[d];
        if (dayData.is_rq || !dayData.is_available) {
          consecutiveWork = 0;
          consecutiveOff++;
          continue;
        }
      ...
      // 규칙 13: is_business_trip은 근무일 수에 포함
      ...
      // 규칙 2: 2일 이상 연속 휴무 우선
      if (consecutiveOff > 0 && consecutiveOff < 2 && !hasLongBreak) {
        schedule[name][d] = "OFF";
        consecutiveWork = 0;
        consecutiveOff++;
        if (consecutiveOff >= 2) hasLongBreak = true;
        continue;
      }
      
      // 규칙 9: 근무일 수 목표 달성 시 OFF
      if (workDays >= targetWorkdays) {
        schedule[name][d] = "OFF";
        consecutiveWork = 0;
        consecutiveOff++;
        continue;
    ...
    while (workDays < targetWorkdays) {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        if (schedule[name][d] === "OFF" && employees[name].days[d] && 
            !employees[name].days[d].is_rq && employees[name].days[d].is_available &&
            canWorkOnDay_(schedule, name, d, prevMonthData)) {
          schedule[name][d] = "ON";
          workDays++;
          assigned = true;
          if (workDays >= targetWorkdays) break;
        }
      }
      if (!assigned) break;
    }
    ...
    if (empType !== "SP" && empType !== "SSV") {
      ...
      if (targetWorkdays) {
        ...
        if (actualWorkdays !== targetWorkdays) {
          violations.push(name + ": 근무일 수 불일치 (목표: " + targetWorkdays + "일, 실제: " + actualWorkdays + "일)");
        }
      }
    }
  }
